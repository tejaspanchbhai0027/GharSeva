...
export default function AdminDashboard() {
  ...
  const handleModerateReview = async (reviewId, isHidden) => {
    try {
      const res = await api.patch(`/reviews/${reviewId}/moderate`, { is_hidden: isHidden });
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-center gap-4 border-b border-slate-800 pb-2">
              <button 
                ...
                className={`font-semibold text-sm pb-2 border-b-2 transition-colors ${activeTab === 'bookings' ? 'border-rose-500 text-rose-400' : 'border-transparent text-slate-400 hover:text-slate-300'}`}
              >
                All Bookings
              </button>
              <button 
                onClick={() => setActiveTab('reviews')}
                className={`font-semibold text-sm pb-2 border-b-2 transition-colors ${activeTab === 'reviews' ? 'border-rose-500 text-rose-400' : 'border-transparent text-slate-400 hover:text-slate-300'}`}
              >
                Content Moderation
              </button>
            </div>
              ...
              <div className="rounded-2xl bg-slate-900/40 border border-slate-800 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-sm">Review Moderation</span>
                  </div>
                ...
                {reviews.length === 0 ? (
                  <div className="p-10 text-center">
                    ...
                    <p className="text-sm font-semibold text-slate-400">No reviews to moderate</p>
                  </div>
                ) : (
                  <div className="divide-y divide-slate-800/50 max-h-[600px] overflow-y-auto">
                    {reviews.map(review => (
                      <div key={review.review_id} className={`p-6 transition ${review.is_hidden ? 'bg-rose-500/5 opacity-80' : 'hover:bg-slate-800/20'}`}>
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <div className="flex items-center gap-1 mb-1">
                              ...
                              {review.is_hidden && (
                                <span className="ml-2 px-2 py-0.5 rounded bg-rose-500/20 text-rose-400 text-[10px] font-bold uppercase tracking-wider">Hidden</span>
                              )}
                            </div>
                            <p className="text-xs text-slate-500">Posted on {new Date(review.created_at).toLocaleDateString()}</p>
                          </div>
                          <button
                            onClick={() => handleModerateReview(review.review_id, !review.is_hidden)}
                          ...
                          >
                            {review.is_hidden ? <><Eye className="w-3.5 h-3.5" /> Restore</> : <><EyeOff className="w-3.5 h-3.5" /> Hide</>}
                          </button>
                        </div>
                        {review.comment && <p className={`text-sm mt-2 ${review.is_hidden ? 'text-slate-500 italic' : 'text-slate-300'}`}>"{review.comment}"</p>}
                        
                        {review.reply && (
                          <div className="bg-slate-800/50 p-3 rounded-lg border-l-2 border-slate-600 mt-3">
                            <p className="text-[10px] text-slate-400 font-bold uppercase mb-1">Provider Reply</p>
                            <p className={`text-sm ${review.is_hidden ? 'text-slate-500' : 'text-slate-300'}`}>{review.reply}</p>
                          </div>
                        )}
                      </div>
                    ))}
